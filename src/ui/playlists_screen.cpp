...
void PlaylistsScreen::render(const Renderer& r)
{
    const int w = r.terminal().width();
    const int h = r.terminal().height();
    r.text(1, 1, Renderer::bold(title()));
    r.hline(2, 1, w);
    if (loading_) r.text(3, 1, Renderer::dim("Loading playlists..."));
    else if (!statusMsg_.empty()) r.text(3, 1, Renderer::dim(statusMsg_));
    else if (continuationToken_) r.text(3, 1, Renderer::dim(std::to_string(items_.size()) + " playlists — a/A queue, Enter play, r reload, > for more"));
    else r.text(3, 1, Renderer::dim(std::to_string(items_.size()) + " playlists — a/A queue, Enter play, r reload"));
    int firstRow = 5;
    int visible = h - 6;
    if (visible < 1) visible = 1;
    if (items_.empty() && !loading_) r.text(firstRow, 1, Renderer::dim("No playlists. Press r to reload."));
    else {
        size_t top = 0;
    ...
    std::string footer = std::string("j/k:move  gg/G:top/bottom  a/A:queue  l/Enter:play  r:reload") + (continuationToken_ ? "  >:more" : "");
    r.text(h - 1, 1, Renderer::dim(footer));
}

bool PlaylistsScreen::handleKey(const Key& key)
{
    ...
    if (keymap::isOpen(key)) {
        ...
        player_->play();
        statusMsg_ = "Playing playlist: " + items_[selected_].display();
        return true;
    }
    return false;
}

void PlaylistsScreen::moveDown() { if (!items_.empty() && selected_ + 1 < items_.size()) ++selected_; }
void PlaylistsScreen::moveUp() { if (selected_ > 0) --selected_; }
void PlaylistsScreen::goTop() { selected_ = 0; }
void PlaylistsScreen::goBottom() { if (!items_.empty()) selected_ = items_.size() - 1; }
